    ...
    public class AccessControl : FunctionModule
    {
            ...
            for (int idx = 0; idx < amount; ++idx)
            {
                ...
                foreach (string accessGroupID in accessGroupIDs)
                {
                    if (accessGroupID.Length > 0)
                    {
                        ...
                        if (UInt32.TryParse(accessGroupID, out item))
                        {
                            accessGroup.accessLevels[accessGroup.numAccessLevels++] = item;
                        }
                    }
                }
        ...
        public void setAccessLevel(IntPtr sdkContext, uint deviceID, bool isMasterDevice)
        {
            ...
            for (int idx = 0; idx < amount; ++idx)
            {
                ...
                Console.WriteLine("  Enter the ID for the access level which you want to set");
                Console.Write("  >>>> ");
        ...
        {
            IntPtr accessScheduleObj = IntPtr.Zero;
            ...
            if (Util.IsYes())
            {
                ...
                result = (BS2ErrorCode)API.BS2_GetAllAccessSchedule(sdkContext, deviceID, out accessScheduleObj, out numAccessSchedule);
            }
            else
            {
                ...
                {
                    IntPtr accessScheduleIDObj = Marshal.AllocHGlobal(4 * accessScheduleIDList.Count);
                    IntPtr curAccessScheduleIDObj = accessScheduleIDObj;
                    foreach (UInt32 item in accessScheduleIDList)
                    {
                        Marshal.WriteInt32(curAccessScheduleIDObj, (Int32)item);
                        curAccessScheduleIDObj = (IntPtr)((long)curAccessScheduleIDObj + 4);
                    }

                    Console.WriteLine("Trying to get access schedules from device.");
                    result = (BS2ErrorCode)API.BS2_GetAccessSchedule(sdkContext, deviceID, accessScheduleIDObj, (UInt32)accessScheduleIDList.Count, out accessScheduleObj, out numAccessSchedule);
            ...
            {
                IntPtr curAccessLevelObj = accessScheduleObj;
                int structSize = Marshal.SizeOf(typeof(BS2Schedule));

                for (int idx = 0; idx < numAccessSchedule; ++idx)
                {
                    BS2Schedule item = (BS2Schedule)Marshal.PtrToStructure(curAccessLevelObj, typeof(BS2Schedule));
                    print(sdkContext, item);

                    curAccessLevelObj = (IntPtr)((long)curAccessLevelObj + structSize);
                }

                API.BS2_ReleaseObject(accessScheduleObj);
            }
            else
            {
        ...
        public void removeAccessSchedule(IntPtr sdkContext, uint deviceID, bool isMasterDevice)
        {
            ...
            else
            {
                ...
                if (accessScheduleIDList.Count > 0)
                {
                    ...
                    Console.WriteLine("Trying to remove access schedules from device.");
                    result = (BS2ErrorCode)API.BS2_RemoveAccessSchedule(sdkContext, deviceID, accessScheduleIDObj, (UInt32)accessScheduleIDList.Count);
        ...
        public void setAccessSchedule(IntPtr sdkContext, uint deviceID, bool isMasterDevice)
        {
            ...
            char[] delimiterChars = { ' ', ',', '.', ':', '\t' };
            int amount = Util.GetInput(1);
            List<BS2Schedule> accessScheduleList = new List<BS2Schedule>();

            for (int idx = 0; idx < amount; ++idx)
            {
                BS2Schedule accessSchedule = Util.AllocateStructure<BS2Schedule>();
            ...
            int structSize = Marshal.SizeOf(typeof(BS2Schedule));
            IntPtr accessScheduleListObj = Marshal.AllocHGlobal(structSize * accessScheduleList.Count);
            IntPtr curAccessScheduleListObj = accessScheduleListObj;
            foreach (BS2Schedule item in accessScheduleList)
            {
                Marshal.StructureToPtr(item, curAccessScheduleListObj, false);
                curAccessScheduleListObj = (IntPtr)((long)curAccessScheduleListObj + structSize);
            }

            Console.WriteLine("Trying to set access schedules to device.");
            BS2ErrorCode result = (BS2ErrorCode)API.BS2_SetAccessSchedule(sdkContext, deviceID, accessScheduleListObj, (UInt32)accessScheduleList.Count);
            if (result != BS2ErrorCode.BS_SDK_SUCCESS)
            {
                Console.WriteLine("Got error({0}).", result);
            }
        ...
        void print(IntPtr sdkContext, BS2AccessGroup accessGroup)
        {
            ...
            for (byte loop = 0; loop < accessGroup.numAccessLevels; ++loop)
            {
                Console.WriteLine("     |  |--accessLevelID[{0}]", accessGroup.accessLevels[loop]);
            }
        }
        ...
        void print(IntPtr sdkContext, BS2Schedule schedule)
        {
