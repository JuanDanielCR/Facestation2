...
namespace Suprema
{
    class DataBaseHandler : IDisposable
    {
        ...
        SQLiteConnection openDataBase()
        {
            ...
            cmd.ExecuteNonQuery();
            cmd.CommandText = "CREATE TABLE IF NOT EXISTS BS2Face(userID CHAR(32), faceIndex INTEGER, templateIndex INTEGER, data BLOB UNIQUE, FOREIGN KEY(userID) REFERENCES User(id) ON DELETE CASCADE)";
            cmd.ExecuteNonQuery();
        ...
        public bool GetUserBlob(ref BS2SimpleDeviceInfo deviceInfo, ref BS2User targetUser, ref BS2UserBlob userBlob)
        {
            ...
            if (targetUser.numFaces > 0)
            {
                ...
                while (rdr.Read())
                {
                    ...
                    curFaceObjs += 3;
                    Marshal.Copy(templateData, 0, curFaceObjs, BS2Envirionment.BS2_TEMPLATE_PER_FACE * BS2Envirionment.BS2_FACE_TEMPLATE_SIZE);
                    curFaceObjs += BS2Envirionment.BS2_TEMPLATE_PER_FACE * BS2Envirionment.BS2_FACE_TEMPLATE_SIZE;
                }
                rdr.Close();
            }
            else
        ...
        public bool AddUserBlob(ref BS2UserBlob userBlob, BS2FingerprintTemplateFormatEnum templateFormat)
        {
            ...
            if (userBlob.user.numFaces > 0)
            {
                ...
                cmd.CommandText = "INSERT INTO BS2Face (userID, faceIndex, templateIndex, data) VALUES (@userIDParam, @faceIndexParam, @templateIndexParam, @dataParam)";
                ...
                for (byte idx = 0; idx < userBlob.user.numFaces; ++idx)
                {
                    ...
                    cmd.Parameters.AddWithValue("@templateIndexParam", face.templateIndex);
                    cmd.Parameters.AddWithValue("@dataParam", face.data);
                ...
                if (userBlob.user.numFingers > 0)
                {
                    ...
                    for (int idx = 0; idx < userBlob.user.numFingers; ++idx)
                    {
                        ...
                        for (UInt32 templateIndex = 0; templateIndex < BS2Envirionment.BS2_TEMPLATE_PER_FINGER; )
                        {
                            ...
                            result = (BS2ErrorCode)API.BS2_ScanFingerprint(sdkContext, deviceID, ref fingerprint, templateIndex, (UInt32)BS2FingerprintQualityEnum.QUALITY_STANDARD, (byte)templateFormat, cbFingerOnReadyToScan);
                            if (result != BS2ErrorCode.BS_SDK_SUCCESS)
                            {
            ...
            if (result == BS2ErrorCode.BS_SDK_SUCCESS)
            {
                if (numUserIds > 0)
                {
                    ...
                    for (UInt32 idx = 0; idx < numUserIds; )
                    {
                        ...
                        result = (BS2ErrorCode)API.BS2_GetUserDatas(sdkContext, deviceID, curUidObjs, available, userBlobs, (UInt32)BS2UserMaskEnum.DATA);
                        if (result == BS2ErrorCode.BS_SDK_SUCCESS)
                        {
