...
namespace Suprema
{
    ...
    class DataBaseHandler : IDisposable
    {
        ...
        public bool AddUserBlob(ref BS2SimpleDeviceInfo deviceInfo, ref BS2UserBlob userBlob, BS2FingerprintTemplateFormatEnum templateFormat)
        {
            ...
            if (Convert.ToBoolean(deviceInfo.fingerSupported) && userBlob.user.numFaces > 0)
            {
                ...
                if (userBlob.user.numFingers > 0)
                {
                    ...
                    for (int idx = 0; idx < userBlob.user.numFingers; ++idx)
                    {
                        ...
                        for (UInt32 templateIndex = 0; templateIndex < BS2Envirionment.BS2_TEMPLATE_PER_FINGER; )
                        {
                            ...
                            result = (BS2ErrorCode)API.BS2_ScanFingerprint(sdkContext, deviceID, ref fingerprint, templateIndex, (UInt32)BS2FingerprintQualityEnum.QUALITY_STANDARD, (byte)BS2FingerprintTemplateFormatEnum.FORMAT_SUPREMA, cbFingerOnReadyToScan);
                            if (result != BS2ErrorCode.BS_SDK_SUCCESS)
                            {
